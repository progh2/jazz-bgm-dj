// bgm-scenes.js — auto-generated 2026-09-08
// 재즈·라운지 장면 분류(34종) 기반 무료 라이선스 BGM 목록.
// focus: 1(집중 방해) ~ 5(작업용 최적)
//
// 전송량을 줄이려고 아티스트·라이선스는 표로 빼고 트랙은 색인만 갖는다.
// import 시점에 원래 객체 형태로 되돌리므로 쓰는 쪽 코드는 바뀌지 않는다:
//   { id, title, artist, videoId, focus, length, license }

export const SCENES = [
  { id: "A1_curtain", category: "오프닝·신호", name: "커튼콜 / 개장", count: 1 },
  { id: "A2_intro", category: "오프닝·신호", name: "밴드 인트로 / 테마", count: 3 },
  { id: "A3_break", category: "오프닝·신호", name: "브레이크 / 인터미션", count: 2 },
  { id: "A4_close", category: "오프닝·신호", name: "라스트콜 / 클로징", count: 1 },
  { id: "B1_swing", category: "장르·결", name: "스윙 / 빅밴드", count: 18 },
  { id: "B2_bebop", category: "장르·결", name: "비밥", count: 5 },
  { id: "B3_cool", category: "장르·결", name: "쿨 재즈", count: 2 },
  { id: "B4_hardbop", category: "장르·결", name: "하드밥", count: 4 },
  { id: "B5_modal", category: "장르·결", name: "모달", count: 4 },
  { id: "B6_latin", category: "장르·결", name: "라틴 / 보사 / 삼바", count: 16 },
  { id: "B7_blues", category: "장르·결", name: "재즈 블루스", count: 17 },
  { id: "B8_ballad", category: "장르·결", name: "발라드", count: 3 },
  { id: "B9_smooth", category: "장르·결", name: "스무스 / 컨템포러리", count: 45 },
  { id: "B10_fusion", category: "장르·결", name: "퓨전 / 일렉트릭", count: 10 },
  { id: "B11_free", category: "장르·결", name: "프리 / 아방가르드", count: 2 },
  { id: "B12_dixie", category: "장르·결", name: "딕실랜드 / 트래디셔널", count: 6 },
  { id: "B13_soul", category: "장르·결", name: "소울 재즈 / 펑키", count: 34 },
  { id: "B14_noir", category: "장르·결", name: "누아르 / 탐정", count: 12 },
  { id: "C1_lounge", category: "장소", name: "미드나잇 라운지", count: 17 },
  { id: "C2_cafe", category: "장소", name: "브런치 카페", count: 4 },
  { id: "C3_speakeasy", category: "장소", name: "스피크이지", count: 2 },
  { id: "C4_rooftop", category: "장소", name: "루프탑 / 시티 라이트", count: 2 },
  { id: "C5_festival", category: "장소", name: "야외 페스티벌", count: 2 },
  { id: "C6_studio", category: "장소", name: "스튜디오 세션", count: 3 },
  { id: "C7_dance", category: "장소", name: "댄스플로어", count: 1 },
  { id: "D1_melancholy", category: "감정·시간", name: "멜랑콜리 / 블루", count: 1 },
  { id: "D2_romance", category: "감정·시간", name: "로맨스", count: 3 },
  { id: "D3_uplift", category: "감정·시간", name: "업리프트 / 선데이 모닝", count: 5 },
  { id: "D4_tense", category: "감정·시간", name: "텐스 / 서스펜스", count: 4 },
  { id: "D5_playful", category: "감정·시간", name: "플레이풀 / 위트", count: 1 },
  { id: "D6_elegant", category: "감정·시간", name: "엘레강스 / 블랙타이", count: 6 },
  { id: "D7_rainy", category: "감정·시간", name: "비 오는 창가", count: 2 },
  { id: "D8_afterhours", category: "감정·시간", name: "애프터아워 / 라스트 세트", count: 1 },
  { id: "D9_dawn", category: "감정·시간", name: "새벽 / 빈 클럽", count: 2 },
];

const A = ["Kevin MacLeod", "MusMus", "TeknoAXE", "Audionautix / Jason Shaw", "Eric Matyas (Soundimage.org)", "Twin Musicom", "Loyalty Freak Music / Komiku", "魔王魂 (森田交一)"];
const L = ["CC BY 4.0", "Free with attribution (musmus.main.jp)", "Free with attribution", "CC0", "Free with attribution (maou.audio terms)"];

/** [id, title, artistIdx, videoId, focus, length, licenseIdx] */
const RAW = {
  // 오프닝·신호 / 커튼콜 / 개장
  "A1_curtain": [
    ["a1_curtain-kwu7wrw3muy", "Hot Swing", 0, "kWU7wrW3muY", 3, 51, 0],
  ],
  // 오프닝·신호 / 밴드 인트로 / 테마
  "A2_intro": [
    ["a2_intro-ftidfrnadqi", "Ryno's Theme - .com", 0, "FTIdFrNaDqI", 4, 182, 0],
    ["a2_intro-ryuynwq38x0", "dear Dragon Example of use: 100% Orange Juice Sweet Breaker's Theme etc.", 1, "rYUynwQ38x0", 4, 195, 1],
    ["a2_intro-7hjr76oc9u0", "'s - (Away and then Home) Intro Funky Music", 2, "7HjR76oC9u0", 2, 99, 0],
  ],
  // 오프닝·신호 / 브레이크 / 인터미션
  "A3_break": [
    ["a3_break-5hpus4kcrfs", "Vibe Ace", 0, "5hPus4kCrFs", 5, 64, 0],
    ["a3_break-3fmbcuoroby", "Bass Soli", 0, "3fMbCuORobY", 4, 66, 0],
  ],
  // 오프닝·신호 / 라스트콜 / 클로징
  "A4_close": [
    ["a4_close-dyxj27za0dq", "Close to Midnight - Electro Swing/Swing Hop", 2, "dyXJ27za0dQ", 5, 200, 0],
  ],
  // 장르·결 / 스윙 / 빅밴드
  "B1_swing": [
    ["b1_swing-xqo5gppa2vi", "As I Figure", 0, "XQo5Gppa2vI", 4, 217, 0],
    ["b1_swing-3szoxbcjnle", "Big Swing Band", 3, "3SZoXbcJNlE", 4, 111, 0],
    ["b1_swing-v0gc7fience", "Don't Mean a Thing - Electro Swing", 2, "v0Gc7FIENcE", 4, 207, 0],
    ["b1_swing-ddmilm0cxg4", "George Street Shuffle", 0, "DDmILM0cxg4", 4, 180, 0],
    ["b1_swing-bxdmpakyxrg", "Hep Cats by", 0, "BxDmPAKyxrg", 4, 251, 0],
    ["b1_swing-vli9812n9v8", "Hep cats", 0, "vLI9812N9V8", 4, 250, 0],
    ["b1_swing-tavpcxn-owg", "Hot Foot - Electro Swing", 2, "taVPCxN-oWg", 4, 205, 0],
    ["b1_swing-c1zmyi3otns", "Knock Knock - Electro Swing", 2, "C1zmYI3oTNs", 4, 226, 0],
    ["b1_swing-u-jjecipqty", "Melody Man - Electro Swing", 2, "U-JjEcipQTY", 4, 240, 0],
    ["b1_swing-thbzsphsuto", "New Type Menace - Electro Swing", 2, "THbZSPhSuto", 4, 197, 0],
    ["b1_swing-mqy_ybdyirg", "Old Man of the Mountain - Swing Hop", 2, "mqY_ybDYIrg", 4, 221, 0],
    ["b1_swing-icqdt_oxlsa", "Porch Swing Days - faster", 0, "iCqDt_OxLSA", 4, 187, 0],
    ["b1_swing-8wttod6l5io", "Retro Jazz & Swing | Music Library | Bank Muzyki", 4, "8wttOD6l5Io", 4, 107, 2],
    ["b1_swing-vhij30x3cqu", "Stomping at Midnight", 0, "VhIJ30X3CqU", 4, 122, 0],
    ["b1_swing-jlf_vsom20s", "Stomping at Midnight Vinyl Recording", 0, "jlF_vSoM20s", 4, 119, 0],
    ["b1_swing-rtahc_olxje", "Swing Machine", 0, "Rtahc_olXjE", 4, 138, 0],
    ["b1_swing-1zzqio6jprq", "Wat Dat Dee - Electro Swing", 2, "1Zzqio6jPRQ", 4, 201, 0],
    ["b1_swing-b3kfmtgnbqg", "Big Band Swingin' ( )", 4, "b3kfMTgnBqg", 3, 85, 2],
  ],
  // 장르·결 / 비밥
  "B2_bebop": [
    ["b2_bebop-jdmsi3iokqc", "Awesome Call", 0, "JDMsI3iOKqc", 3, 153, 0],
    ["b2_bebop-rqeyyi9p4bk", "Be Bop 25", 3, "rqeYYi9P4Bk", 2, 143, 0],
    ["b2_bebop-v99vsacpgai", "Dispersion Relation", 0, "v99vSaCpgAI", 2, 245, 0],
    ["b2_bebop-1glncqepziq", "Faster Does It", 0, "1glncQEPzIQ", 2, 184, 0],
    ["b2_bebop-egttmzsexhg", "No Good Layabout", 0, "egTTMzSeXHg", 2, 187, 0],
  ],
  // 장르·결 / 쿨 재즈
  "B3_cool": [
    ["b3_cool-htvctalhgvu", "'s - Royalty Free Song (Laid Back) Elevator Music?", 2, "htvCtAlhGvU", 5, 270, 0],
    ["b3_cool-_q3nyfldjvs", "Natural Vibes", 0, "_q3NYfldJvs", 5, 132, 0],
  ],
  // 장르·결 / 하드밥
  "B4_hardbop": [
    ["b4_hardbop-qcsfjsotauo", "Funky Chunk", 0, "qcSFJSoTauo", 4, 241, 0],
    ["b4_hardbop-ohor51cs_js", "Closer To Jazz", 3, "OHor51cs_Js", 3, 143, 0],
    ["b4_hardbop-thlyvvldah0", "Sidewalk Shade", 0, "thLYvVLdAh0", 3, 183, 0],
    ["b4_hardbop-balbvu8zlvq", "Sidewalk Shade - Note for Note", 0, "balbVu8zlvQ", 3, 157, 0],
  ],
  // 장르·결 / 모달
  "B5_modal": [
    ["b5_modal-ifrf3b2shdu", "Intractable", 0, "IFrf3B2sHDU", 5, 271, 0],
    ["b5_modal-sfxuva3gmia", "Mellowtron", 0, "sfxUvA3GMiA", 5, 54, 0],
    ["b5_modal-blubf0viane", "Shades of Spring", 0, "bLuBF0vianE", 4, 180, 0],
    ["b5_modal-kqcvcoz8wno", "Mellowtron by", 0, "kqcvcoz8Wno", 3, 54, 0],
  ],
  // 장르·결 / 라틴 / 보사 / 삼바
  "B6_latin": [
    ["b6_latin-vygpdi8wgic", "Blobby Samba", 0, "VYgpDi8WGic", 5, 234, 0],
    ["b6_latin-mfsvlwy0tee", "Blobby Samba - Latinesque", 0, "mfSvLWY0teE", 5, 234, 0],
    ["b6_latin-a25zuqfejr4", "Bossa Antigua", 0, "A25zUqFEJr4", 5, 180, 0],
    ["b6_latin-_118wtrzab0", "BossaBossa", 0, "_118WTrzaB0", 5, 168, 0],
    ["b6_latin-t1ugi0cpa50", "Casa Bossa Nova", 0, "T1UgI0CpA50", 5, 243, 0],
    ["b6_latin-o_tcgpofwpo", "Casa Bossa Nova - .com", 0, "O_TcGPoFwPo", 5, 245, 0],
    ["b6_latin-lgw3u0wa3vg", "Falling Leaves / Jazz Bossa /", 1, "lgW3U0WA3vg", 5, 249, 1],
    ["b6_latin-7ge3pwsfu48", "Hot Salsa", 3, "7ge3pWSFu48", 5, 141, 0],
    ["b6_latin-iagw8l5s-xa", "Modern Jazz Samba", 0, "IaGW8L5S-XA", 5, 251, 0],
    ["b6_latin-iay1fwiigwc", "Off to Osaka", 0, "iaY1fWiiGwc", 5, 109, 0],
    ["b6_latin-hpsgkm3n2am", "Old Bossa", 5, "hPSgKm3n2aM", 5, 161, 0],
    ["b6_latin-8fbvi38gvjq", "Sad Bossa /", 1, "8fbvI38gVJQ", 5, 139, 1],
    ["b6_latin-jy81kvouwpo", "Samba Isobel", 0, "JY81kVOuwPo", 5, 251, 0],
    ["b6_latin-ltblurpmmdy", "Sideways Samba", 3, "ltblurPMmdY", 5, 191, 0],
    ["b6_latin-2a1m4orjy2k", "柔らかな嘘", 1, "2a1M4ORJY2k", 5, 172, 1],
    ["b6_latin-ou-moaztxhe", "Samba Stings", 0, "OU-MOAZtXHE", 3, 47, 0],
  ],
  // 장르·결 / 재즈 블루스
  "B7_blues": [
    ["b7_blues-zoai7wds44c", "'s - Basic Blues Stomp -- Rock/Retro", 2, "zOai7WDS44c", 4, 167, 0],
    ["b7_blues-33wptzmunai", "Acoustic Blues", 3, "33WPtZmunaI", 4, 153, 0],
    ["b7_blues-gtl6kydlgps", "Bass Walker", 0, "gTl6KydlGps", 4, 163, 0],
    ["b7_blues-ddjqcewyu1c", "Bass Walker - Film Noir", 0, "ddjQcewYu1c", 4, 162, 0],
    ["b7_blues-nzexs165t7c", "Big Blues", 3, "nZexS165t7c", 4, 202, 0],
    ["b7_blues-mehoyemjtbk", "Big Blues Bed", 3, "meHoyeMJTbk", 4, 195, 0],
    ["b7_blues-h1rzv2uhfbk", "Double Blues", 3, "H1rZv2uHfbk", 4, 127, 0],
    ["b7_blues-91urklvsq9e", "Matt's Blues", 0, "91URklVsq9E", 4, 169, 0],
    ["b7_blues-1_jpevyost4", "Monplaisir - Well end blues", 6, "1_jPEVyost4", 4, 160, 3],
    ["b7_blues-pgh9wopoida", "MyCherryCrush Bad Ol' Cat Rainy November Mix -- Jazz/Blues", 2, "pGH9wOPOiDA", 4, 199, 0],
    ["b7_blues-qycgqiiquqc", "Nile's Blues", 0, "QYCgQIiqUqc", 4, 156, 0],
    ["b7_blues-qjcnidvyfz0", "OctoBlues", 0, "qJcnidvyfz0", 4, 259, 0],
    ["b7_blues-25xg93vqmnw", "OctoBlues - .com", 0, "25xg93vqmNw", 4, 253, 0],
    ["b7_blues-lh6rlmhfj18", "Opheliea's Blues", 3, "lh6rlMHFj18", 4, 245, 0],
    ["b7_blues-fvuhgpknh6e", "Porch Blues", 0, "FvUhGpKNH6E", 4, 118, 0],
    ["b7_blues-_45nln96x_s", "Brookers Blues", 3, "_45nLn96x_s", 3, 96, 0],
    ["b7_blues-gqkq_q_hqri", "RPG BLues", 4, "gQKQ_q_hQrI", 3, 90, 2],
  ],
  // 장르·결 / 발라드
  "B8_ballad": [
    ["b8_ballad-kjkhxnci548", "'s - A Lullaby for James -- A Sleepy Song -- PCH Drive", 2, "KJkHxnci548", 5, 214, 0],
    ["b8_ballad-6ydch18nsng", "3 minutes DTM : Electric piano slow jazz - DTM", 1, "6ydCH18Nsng", 5, 174, 1],
    ["b8_ballad-z6j690yzffy", "Smooth Lovin", 0, "z6j690YzFFY", 5, 180, 0],
  ],
  // 장르·결 / 스무스 / 컨템포러리
  "B9_smooth": [
    ["b9_smooth-5fe7qh3kbga", "'s - Royalty Free Song (The Crazyhorse) Zen Song (Circa 2005)", 2, "5fE7QH3kbGA", 5, 273, 0],
    ["b9_smooth-4-xh-xbyfym", "11th finger", 1, "4-XH-XBYfyM", 5, 239, 1],
    ["b9_smooth-04wln7ajk7s", "64 Sundays", 5, "04WlN7aJK7s", 5, 142, 0],
    ["b9_smooth--lw6nqtxe10", "Alpha Hydrae- Keratine, rust and clear soul", 6, "-LW6nQtXe10", 5, 304, 3],
    ["b9_smooth-wj_fj8g_pz0", "Chillin' with My Puzzles", 4, "wj_fj8G_PZ0", 5, 144, 2],
    ["b9_smooth-kc2do80ljge", "City Moon", 4, "Kc2dO80lJgE", 5, 124, 2],
    ["b9_smooth-5c1say_pijy", "Crushin", 3, "5c1say_pIjY", 5, 256, 0],
    ["b9_smooth-m5olymxlfx0", "Dream Voyager", 4, "M5olymXlFx0", 5, 142, 2],
    ["b9_smooth-auqlm6kx9qq", "Ethernight Club", 0, "aUQLM6kX9qQ", 5, 307, 0],
    ["b9_smooth-f6cccc-xvmy", "Gulf Breeze", 4, "F6cccC-xvMY", 5, 143, 2],
    ["b9_smooth-ytuk90ip4by", "Haunting Dreams", 5, "Ytuk90IP4BY", 5, 168, 0],
    ["b9_smooth-r6bdisixori", "Kat and Tanner", 5, "r6BdiSixOrI", 5, 135, 0],
    ["b9_smooth-qj3slui0v5q", "Koichi Morita - Mirage (Sore ha Shinkiro datta)", 7, "QJ3SluI0V5Q", 5, 293, 4],
    ["b9_smooth-4wwypyji-tw", "L'orage et l'aventure", 6, "4wwypyji-Tw", 5, 212, 3],
    ["b9_smooth-wj0yu9hxkaa", "Leopard Print Elevator", 0, "wJ0Yu9hxkaA", 5, 330, 0],
    ["b9_smooth-vs5seovgwrc", "Leopard Print Elevator - .com", 0, "vS5seoVGwrc", 5, 330, 0],
    ["b9_smooth-owhr4ebsxee", "Leopard Print Elevator by", 0, "oWHr4ebSxEE", 5, 328, 0],
    ["b9_smooth-tpw_g5zb07m", "Mall Walker", 3, "tPw_G5ZB07M", 5, 273, 0],
    ["b9_smooth-xpjpgnyrcag", "Nice Old Tradition", 1, "xPjpGnYrCAg", 5, 203, 1],
    ["b9_smooth-rdxiej3apms", "Not Without the Rest", 5, "RDxieJ3Apms", 5, 118, 0],
    ["b9_smooth-2vahb2iqqgg", "Picnic Breeze", 4, "2vAhB2IqqGg", 5, 100, 2],
    ["b9_smooth-nbbj9weexq0", "Port Horizon", 0, "nbbJ9weexQ0", 5, 324, 0],
    ["b9_smooth-2uov7gzeslk", "Royalty Free Background Music (Chillaxing with the Saxing) Downtempo/Chill/Jazz", 2, "2Uov7GZeslk", 5, 315, 0],
    ["b9_smooth-iqxdqji_49c", "Royalty Free Background Music -A (Floating on Glass Super Chill Mode) Downtempo/Chill", 2, "IqXDqji_49c", 5, 270, 0],
    ["b9_smooth-w2jbfjg3vgc", "Sheba out on the Town -- Downtempo/Jazz", 2, "W2jbFjg3vgc", 5, 247, 0],
    ["b9_smooth-5vx9a0xtc6i", "Space Jazz", 0, "5vX9A0XTc6I", 5, 370, 0],
    ["b9_smooth-nqv0o7hva4i", "Straighty Baby", 3, "NqV0o7hVa4I", 5, 215, 0],
    ["b9_smooth-rstmmm-8w1i", "Stratosphere", 4, "RsTmmM-8W1I", 5, 199, 2],
    ["b9_smooth-fvpvz2uwdb8", "THE ミュラ ー", 1, "fVpVz2UWdb8", 5, 238, 1],
    ["b9_smooth-oerdo5keu3e", "Tiki Breeze", 4, "oERdo5kEU3E", 5, 129, 2],
    ["b9_smooth--jg7v3e3lew", "Tom Perry", 5, "-JG7V3E3Lew", 5, 164, 0],
    ["b9_smooth-n2y4cz4ltbg", "Touchpoint", 3, "n2y4cZ4Ltbg", 5, 290, 0],
    ["b9_smooth-gwqkv76azgi", "Triangle", 3, "gWQKv76azGI", 5, 221, 0],
    ["b9_smooth-lfsn0blcuqa", "Wheels", 3, "lfsn0BlCUqA", 5, 212, 0],
    ["b9_smooth-wm7fxpwi-ly", "With a Stamp", 5, "Wm7fXPwI-LY", 5, 282, 0],
    ["b9_smooth-tgmdouoy-yk", "turns into George Lucas...again", 2, "tGMDouOy-yk", 5, 665, 0],
    ["b9_smooth-ecfvi2ebav0", "ジャズで聴くファイナルファンタジー4", 1, "EcfVi2EBaV0", 5, 196, 1],
    ["b9_smooth-clzw4fdx7ug", "卒業（体育館 ）", 1, "CLzw4fDx7ug", 5, 236, 1],
    ["b9_smooth-mfawighzage", "卒業（音楽室 ）", 1, "MfaWiGHZAgE", 5, 236, 1],
    ["b9_smooth-wz_2ur4j4eo", "怠惰の果実", 1, "wZ_2uR4j4eo", 5, 327, 1],
    ["b9_smooth-rlx6i2fnqk4", "手描きアート Koichi Morita Art 004 Alien Sorting Soul", 7, "rlX6i2fNQK4", 5, 739, 4],
    ["b9_smooth-otgdebate2q", "暗いジャズのアコースティック02", 7, "otGDEbATe2Q", 5, 105, 4],
    ["b9_smooth-3xjqtmpqvoc", "Classical Carnivale", 5, "3XJQtMPQvoc", 4, 91, 0],
    ["b9_smooth-h3ylvrgmkam", "ややジャズぃ〜なアコースティック15", 7, "h3YLVRGmkaM", 4, 61, 4],
    ["b9_smooth-slhv2v6c64u", "心地よいジャズのアコースティック11", 7, "slhV2v6C64U", 4, 87, 4],
  ],
  // 장르·결 / 퓨전 / 일렉트릭
  "B10_fusion": [
    ["b10_fusion-kisszsqu7nq", "Cold Funk - Funkorama", 0, "KISsZSqU7nQ", 4, 289, 0],
    ["b10_fusion-dmio-dt5bfw", "Funkorama", 0, "dmIO-dT5BFw", 4, 203, 0],
    ["b10_fusion-wrw5uaagnw8", "Local Forecast", 0, "wRw5uAAGnw8", 4, 190, 0],
    ["b10_fusion-5iyl1l34pis", "Local Forecast - Elevator", 0, "5IyL1l34PIs", 4, 192, 0],
    ["b10_fusion-3jvixh3tmp0", "Local Forecast - Slower", 0, "3JVIxh3tMp0", 4, 201, 0],
    ["b10_fusion-deb5zrrsaz4", "Local Forecast Elevator", 0, "dEb5zRrSAZ4", 4, 190, 0],
    ["b10_fusion-kjmaelp4lvi", "AcidJazz", 0, "kJmAelp4lVI", 3, 180, 0],
    ["b10_fusion-x5cdq4zyzom", "C-Funk - Funkorama", 0, "X5cDq4ZyZoM", 3, 171, 0],
    ["b10_fusion-gimbmtwbv1s", "Laser Groove", 0, "GImBmTwBV1s", 3, 168, 0],
    ["b10_fusion-hywhvtkxo6u", "Local Forecast (Normal)", 0, "HYWhvTkXO6U", 3, 166, 0],
  ],
  // 장르·결 / 프리 / 아방가르드
  "B11_free": [
    ["b11_free-urgd-inuywy", "2017.12.08 Atonal", 0, "uRGd-INuYWY", 1, 344, 0],
    ["b11_free-ke3wr5d1fua", "Atonal in C-ish", 0, "kE3WR5d1fuA", 1, 42, 0],
  ],
  // 장르·결 / 딕실랜드 / 트래디셔널
  "B12_dixie": [
    ["b12_dixie-b1wiejj99xa", "Fig Leaf Rag - distressed", 0, "B1WIEJj99XA", 4, 212, 0],
    ["b12_dixie-01afmksqzu0", "Fig Leaf Times Two", 0, "01AFmkSqZu0", 4, 381, 0],
    ["b12_dixie-uahft4fw0hm", "2007 04 & Dan Beimborn - Ragtime Annie", 0, "uaHFt4Fw0HM", 3, 160, 0],
    ["b12_dixie-vhh1qd6-r0m", "Fig Leaf Rag", 0, "vHh1Qd6-r0M", 3, 211, 0],
    ["b12_dixie--2l1yqnjuku", "Frogs Legs Rag", 0, "-2L1YqnJuKU", 3, 169, 0],
    ["b12_dixie-b1izxvfdjis", "Maple Leaf Rag", 0, "B1izxvFdJis", 3, 152, 0],
  ],
  // 장르·결 / 소울 재즈 / 펑키
  "B13_soul": [
    ["b13_soul-sk883ji5uas", "'s - (Resonance Experiment) Funky Big Beat Music", 2, "Sk883ji5Uas", 4, 257, 0],
    ["b13_soul-1dgxozonlgi", "'s - Royalty Free Jam Music (Speaker Stomp Boogie) Rock/Retro/Hard", 2, "1dGXozoNlGI", 4, 199, 0],
    ["b13_soul-advszjs_n_g", "'s - Royalty Free Song (Transient) Get Funky Song", 2, "adVSZJS_n_g", 4, 320, 0],
    ["b13_soul-l7wtgxtjntc", "Boogie Party", 0, "l7WTgXTjNtc", 4, 318, 0],
    ["b13_soul-dmzt2cnilvu", "Clean Soul", 0, "DmZT2cniLVU", 4, 309, 0],
    ["b13_soul-thihnuqjhf4", "Clean Soul (EP and Bass Duet)", 0, "THihnuQJHF4", 4, 307, 0],
    ["b13_soul-m822dtfvpae", "Clean Soul - Calming", 0, "M822DtfVpaE", 4, 307, 0],
    ["b13_soul-0gp8kimr1zy", "Cold Funk", 0, "0GP8kImr1zY", 4, 291, 0],
    ["b13_soul-eb1zg49ulio", "Funk Japan / Copyright", 1, "EB1zg49Ulio", 4, 207, 1],
    ["b13_soul-yicqegdwca0", "Funk You O - Funk/Alternative", 2, "yiCQeGdwcA0", 4, 232, 0],
    ["b13_soul-mk1l2xyxuv4", "Funk the Chilldown - Funk/Chill", 2, "mK1L2xyXuV4", 4, 258, 0],
    ["b13_soul-kixdwb3ksh8", "Funky Boxstep", 0, "KixDwB3ksh8", 4, 316, 0],
    ["b13_soul-s9qlz6dviwu", "Funky One", 0, "S9QLZ6dVIWU", 4, 183, 0],
    ["b13_soul-v3e3_pfja44", "Got Funk", 0, "v3E3_pFJA44", 4, 282, 0],
    ["b13_soul-f-k7kk7xeim", "Groove Grove", 0, "f-k7Kk7XeIM", 4, 207, 0],
    ["b13_soul-xqaozlds9kc", "Groove Grove (with composer commentary)", 0, "XqAozLDS9kc", 4, 206, 0],
    ["b13_soul-iujdybezofk", "Groove Grove - .com", 0, "IUJdYbeZoFk", 4, 206, 0],
    ["b13_soul-81nl9dgoewa", "Jam the Future - Funk/Alternative", 2, "81NL9DGoeWA", 4, 210, 0],
    ["b13_soul-6ccwezvgxtc", "Jumpin Boogie Woogie", 3, "6cCwEZvgXtc", 4, 183, 0],
    ["b13_soul-yypkc7tgrhk", "Moped Cruising -- Breakbeats/Funk", 2, "yYpKC7TgRHk", 4, 253, 0],
    ["b13_soul-_fuxd7mw2xo", "Palm and Soul", 0, "_fuXd7mW2xo", 4, 242, 0],
    ["b13_soul-6eoummyzf9s", "Shifting Sands - Funk/Easy", 2, "6EouMmYZf9s", 4, 233, 0],
    ["b13_soul-58ismd33iuq", "Simpler Times - Funk/Alternative", 2, "58ISMD33IuQ", 4, 226, 0],
    ["b13_soul-wse9bkkdghi", "Boogie Woogie Bed", 3, "wSe9BkkDGhI", 3, 147, 0],
    ["b13_soul-kyvxdeu8fyy", "C-Funk", 0, "KyvXdEu8fYY", 3, 173, 0],
    ["b13_soul-a37a7a2pbwm", "Funky Gameplay", 4, "a37a7A2PBwM", 3, 137, 2],
    ["b13_soul-2lkoboatetg", "Captain Glouglou & their beautiful keyboard - Organ", 6, "2lkoboAtETg", 2, 93, 3],
    ["b13_soul-j5bjczyyhic", "Funky Goat Store", 1, "J5bJCZyyHIc", 2, 61, 1],
    ["b13_soul-xd9v_0kqjc4", "Funky Monster Mischief", 4, "Xd9V_0kQjC4", 2, 76, 2],
    ["b13_soul-j5yc2j9kjgu", "Funky Runnin'", 4, "j5yc2j9KjgU", 2, 93, 2],
    ["b13_soul-d4kfviefv6m", "Future Funk", 4, "D4KFvIefV6M", 2, 86, 2],
    ["b13_soul-w8-brfquku4", "Unconventional Organ", 1, "W8-bRfQuku4", 2, 90, 1],
    ["b13_soul-_pvxp9xx05q", "Funk Game", 0, "_pVxP9xX05Q", 1, 59, 0],
    ["b13_soul-hx0cphltu10", "Funky Streets", 4, "hX0cPhLTu10", 1, 51, 2],
  ],
  // 장르·결 / 누아르 / 탐정
  "B14_noir": [
    ["b14_noir-5b9a2w5eftm", "Dances and Dames", 0, "5B9a2W5eFTM", 5, 180, 0],
    ["b14_noir-wfbiexutm6a", "Darkest Child", 0, "WfBIexUTm6A", 5, 239, 0],
    ["b14_noir-fzi1o9dhmzg", "Darkest Child (var A)", 0, "fZi1O9dHMZg", 5, 200, 0],
    ["b14_noir-ncwyta2kt8g", "Darkest Child A", 0, "nCwYta2Kt8g", 5, 198, 0],
    ["b14_noir-3xieyu0qxzg", "I Knew a Guy", 0, "3XiEYu0Qxzg", 5, 180, 0],
    ["b14_noir-njle-047bzc", "Royalty Free Song (Shadow nights W_ Vocals) Downtempo/Electronica Music", 2, "NJle-047bZc", 5, 259, 0],
    ["b14_noir-kbjzxvjezoa", "Spy Groove", 0, "kbjZxvjEzOA", 5, 183, 0],
    ["b14_noir-fx0sguvknns", "The Arcade Detective", 4, "Fx0sGuVknNs", 5, 117, 2],
    ["b14_noir-irgqbbpr1ke", "Hard Boiled", 0, "irgqBbPR1kE", 4, 180, 0],
    ["b14_noir-7_t7gq8olgk", "Investigations", 0, "7_t7gQ8Olgk", 4, 96, 0],
    ["b14_noir-k4vw4lkoejy", "deck brush detective", 1, "k4VW4LkoeJY", 4, 78, 1],
    ["b14_noir-rki9jvgw7ty", "Future Noir", 4, "RKi9JVgW7tY", 3, 55, 2],
  ],
  // 장소 / 미드나잇 라운지
  "C1_lounge": [
    ["c1_lounge-_tro5l3ong0", "10 Past Midnight", 4, "_TRO5l3onG0", 5, 109, 2],
    ["c1_lounge-3jsmyi38bgk", "Airport Lounge", 0, "3jsMYi38bGk", 5, 308, 0],
    ["c1_lounge-vteyvt8imae", "Airport Lounge - Disco Ultralounge", 0, "VtEYvt8IMaE", 5, 308, 0],
    ["c1_lounge-yf_haxd7stk", "BLEND - Guitor,Sax, Relax,Jazz", 1, "yF_HAXd7Stk", 5, 158, 1],
    ["c1_lounge-oyeie3re7dk", "Backbay Lounge", 0, "OYeiE3Re7dk", 5, 267, 0],
    ["c1_lounge-0b__ab9rteo", "Chill - Disco Ultralounge", 0, "0b__Ab9Rteo", 5, 231, 0],
    ["c1_lounge-aqqejbbbdt4", "Disco Ultralounge", 0, "aQQeJBbBdt4", 5, 229, 0],
    ["c1_lounge-qhtl-6cf5xk", "Late Night Radio", 0, "QhtL-6Cf5Xk", 5, 265, 0],
    ["c1_lounge-krwdtoaswdm", "Midnight Mist", 4, "kRWdtoasWdM", 5, 124, 2],
    ["c1_lounge-liwl0zwq2oc", "Moon Lounge Omicron Prime", 0, "liWL0ZwQ2Oc", 5, 225, 0],
    ["c1_lounge-i73dqey-aea", "Mystery Sax", 0, "I73DQEy-aEA", 5, 285, 0],
    ["c1_lounge-5y2icwvl06m", "Night on the Docks - Piano", 0, "5y2iCWVL06M", 5, 177, 0],
    ["c1_lounge-l7iqnw3ia4u", "Ultralounge", 0, "L7iqnW3iA4U", 5, 318, 0],
    ["c1_lounge-6q-ffdlztvm", "Hong Kong Midnight", 4, "6Q-FfDlZtvM", 4, 63, 2],
    ["c1_lounge-fy7ktglkeom", "Midnight Crawlers", 4, "fy7kTGLKeoM", 4, 63, 2],
    ["c1_lounge-rj0jwldy0xw", "Midnight Mist_LoFi ( Track)", 4, "rj0jwLdy0Xw", 3, 48, 2],
    ["c1_lounge-oauis-l37ca", "Midnight Streets_ ( Track)", 4, "oaUis-L37cA", 3, 56, 2],
  ],
  // 장소 / 브런치 카페
  "C2_cafe": [
    ["c2_cafe-8dtq21bwvyq", "Jazz Brunch", 0, "8DtQ21BWvyQ", 5, 180, 0],
    ["c2_cafe-wtfovv1bafk", "Little Monster’s Cafe / Copyright", 1, "WTFoVv1BaFk", 5, 142, 1],
    ["c2_cafe-7fsklw3xyn8", "Lobby Time", 0, "7FsKlW3xYN8", 5, 193, 0],
    ["c2_cafe-c7cfpg08_yo", "Stable yetl - PIano Jazz", 1, "C7CFPG08_Yo", 5, 165, 1],
  ],
  // 장소 / 스피크이지
  "C3_speakeasy": [
    ["c3_speakeasy-l8arj1fxxa4", "Bar Street Jam - Chill/Easy", 2, "L8aRJ1FxXa4", 5, 277, 0],
    ["c3_speakeasy-qwmp_vng-ym", "Whiskey on the Mississippi", 0, "qWmP_Vng-YM", 4, 198, 0],
  ],
  // 장소 / 루프탑 / 시티 라이트
  "C4_rooftop": [
    ["c4_rooftop-snz7hvmq74e", "Digital Downtown", 4, "Snz7hVmq74E", 4, 106, 2],
    ["c4_rooftop--vo4ax9fm-i", "URBAN FLIGHT", 1, "-VO4ax9FM-I", 4, 138, 1],
  ],
  // 장소 / 야외 페스티벌
  "C5_festival": [
    ["c5_festival-1_cvfd2jtpa", "Tiki Bar Mixer", 0, "1_cvfD2jtpA", 4, 213, 0],
    ["c5_festival--9magnskyd0", "Beach Party", 0, "-9mAgnSkYD0", 3, 180, 0],
  ],
  // 장소 / 스튜디오 세션
  "C6_studio": [
    ["c6_studio-aim9xzttkfi", "Bass Vibes", 0, "Aim9XZttkFI", 5, 170, 0],
    ["c6_studio-tjqwsv4cgge", "Cool Vibes", 0, "TjqwSV4CggE", 5, 219, 0],
    ["c6_studio-jcwdxbctu_w", "Hat the Jazz", 5, "JcWDxBCtu_w", 4, 49, 0],
  ],
  // 장소 / 댄스플로어
  "C7_dance": [
    ["c7_dance-027tcvvj4y0", "'s - -A (Chilling on Space Colony 1 The Dance Remix ) Downtempo", 2, "027TcVVj4Y0", 2, 262, 0],
  ],
  // 감정·시간 / 멜랑콜리 / 블루
  "D1_melancholy": [
    ["d1_melancholy-h6ydejhm4ey", "Bittersweet", 0, "H6ydejhm4EY", 5, 202, 0],
  ],
  // 감정·시간 / 로맨스
  "D2_romance": [
    ["d2_romance-htd1sl4g4o8", "Romantic", 0, "HTd1sl4G4O8", 5, 180, 0],
    ["d2_romance-p5qiwdldfs8", "Romantic Halloween Theme", 4, "P5QiwdLdFS8", 5, 101, 2],
    ["d2_romance-ggvrfcooami", "Romantic Lands Beckon", 4, "ggVRfCoOamI", 5, 122, 2],
  ],
  // 감정·시간 / 업리프트 / 선데이 모닝
  "D3_uplift": [
    ["d3_uplift-z4dq5qmabn0", "'s - Royalty Free Song (Lucid Morning) Thematic Music", 2, "z4Dq5qMabN0", 4, 262, 0],
    ["d3_uplift-smcwoajfuw0", "Carefree", 0, "sMcWOaJFuw0", 4, 206, 0],
    ["d3_uplift-axphs1morwo", "Carefree Days in Groovyville", 4, "AxPHs1MoRwo", 4, 109, 2],
    ["d3_uplift-rbj56gngyb0", "気分上々ジャズ", 1, "rBJ56gnGYB0", 4, 208, 1],
    ["d3_uplift-plqvzi9kfwm", "Carefree Days at Pelican Shores", 4, "PlqVzi9kFwM", 3, 92, 2],
  ],
  // 감정·시간 / 텐스 / 서스펜스
  "D4_tense": [
    ["d4_tense-rx9kco_vnsy", "'s - (Dead Piano Island) Tension/Suspense/Orchestra", 2, "rx9KcO_VnSY", 4, 197, 0],
    ["d4_tense-5azmctdbqii", "'s - (Reception) Orchestra/Tension/Drama/Suspense", 2, "5AZMctDbqII", 4, 226, 0],
    ["d4_tense-gtrhdi8iuvs", "Spy Glass", 0, "gTrHdi8iuvs", 4, 226, 0],
    ["d4_tense-iyhefenwack", "'s - (Boss Battle 1) bpm 120 Suspense/Action/Orchestra", 2, "IYhEfEnWaCk", 2, 81, 0],
  ],
  // 감정·시간 / 플레이풀 / 위트
  "D5_playful": [
    ["d5_playful-qrqjlopbnyy", "Sneaky Snitch", 0, "QrqjLoPbnyY", 3, 143, 0],
  ],
  // 감정·시간 / 엘레강스 / 블랙타이
  "D6_elegant": [
    ["d6_elegant-fogsfh0s5uu", "Wallpaper", 0, "fOgsFH0s5uU", 5, 221, 0],
    ["d6_elegant-im3k4nfdywk", "sleepin’ maple syrup jazz", 1, "Im3K4nFdyWk", 5, 73, 1],
    ["d6_elegant-dzkl4---jhm", "George Duff/ /John Martin - 'Belltable Waltz set'", 0, "DzKL4---JhM", 4, 246, 0],
    ["d6_elegant-zbisc7u-2om", "Joey's Formal Waltz", 0, "zBisC7u-2OM", 4, 305, 0],
    ["d6_elegant-otpekonp6us", "古き良き日々", 1, "OtPeKoNp6us", 4, 203, 1],
    ["d6_elegant-3rwrzy2jupa", "Light Up delight", 1, "3rwrZY2JupA", 3, 138, 1],
  ],
  // 감정·시간 / 비 오는 창가
  "D7_rainy": [
    ["d7_rainy-5rqc913k3kg", "Puddle Jumping", 4, "5Rqc913k3kg", 5, 138, 2],
    ["d7_rainy-yanzvd_mgjw", "Rainy November -- Background/Jazz/Orchestra", 2, "yAnzVd_mgjw", 5, 198, 0],
  ],
  // 감정·시간 / 애프터아워 / 라스트 세트
  "D8_afterhours": [
    ["d8_afterhours-baykt6yw23m", "Mining by Moonlight", 0, "bAYKt6yW23M", 5, 196, 0],
  ],
  // 감정·시간 / 새벽 / 빈 클럽
  "D9_dawn": [
    ["d9_dawn-3-vz_wi00se", "Lightless Dawn", 0, "3-vZ_wi00SE", 5, 381, 0],
    ["d9_dawn-n9ee7n03ya4", "The City Without Dawn", 1, "N9EE7n03YA4", 5, 256, 1],
  ],
};

const hydrate = (r) => ({ id: r[0], title: r[1], artist: A[r[2]], videoId: r[3], focus: r[4], length: r[5], license: L[r[6]] });

export const BGM_BY_SCENE = Object.fromEntries(
  Object.entries(RAW).map(([k, v]) => [k, v.map(hydrate)])
);

/** 모든 트랙 (평탄화) */
export const ALL_TRACKS = Object.entries(BGM_BY_SCENE).flatMap(([scene, ts]) => ts.map(t => ({ ...t, scene })));

/** 작업용 기본 재생목록: 집중도 4 이상만, 장면을 섞어 셔플 */
export const FOCUS_PLAYLIST = ALL_TRACKS.filter(t => t.focus >= 4);

/** 장면 id 배열로 재생목록 만들기. 예: buildPlaylist(['B3_cool','C1_lounge'], 4) */
export function buildPlaylist(sceneIds, minFocus = 1) {
  return sceneIds.flatMap(id => (BGM_BY_SCENE[id] || []).filter(t => t.focus >= minFocus));
}

/** 재생기 크레딧 문구 (현재 트랙 기준) */
export function creditFor(track) {
  return track ? `${track.artist} — ${track.license}` : '';
}

/** 전체 크레딧 (한 번에 표기할 때) */
export const CREDITS = [...new Set(ALL_TRACKS.map(t => `${t.artist} (${t.license})`))].sort();
