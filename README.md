...
- **바 카운터와 셋리스트 대** — 노래가 시작되면 MIDNIGHT 네온이 살아나고 턴테이블이 돌며, 곡이 바뀌면 셋리스트 카드가 넘어갑니다. 카드에는 원곡으로 나가는 자리가 붙어 있어, 마음에 든 곡을 만든 사람에게 곧장 갈 수 있습니다.
- **네온 사인과 턴테이블** — CSS 워드마크(MIDNIGHT · OPEN/LIVE)와 플래터·톤암 덱. 재생 중일 때만 밝게 숨 쉽니다.
- **선반 위의 물건들** — 바 선반의 등불은 화면 켜두기, 놋쇠 열쇠는 유튜브 계정 연결의 **상태이자 스위치**입니다.
- **칵테일 조율** — 레시피 카드가 속삭이는 비율(스피릿·시트러스·스위트)을 맞추고 Shake. 성공하면 잔이 레일에 올라 마실 수 있습니다. 틀리면 muddled.
- **합성 소리** — 도어 차임, 네온 스파크, 셰이커, 잔 부딪는 소리, 턴테이블 건드리는 소리 등을 그 자리에서 만듭니다.
...
assets/css/tavern.css   나무·돌 재질, 바 카운터·네온 사인·턴테이블·칵테일 UI
