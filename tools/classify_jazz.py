...
SUB = {
    "A1_curtain": ("오프닝·신호", "커튼콜 / 개장",
        r"\b(curtain|open(ing)? (night|club|set)|doors? open|welcome|house (open|lights)|enter|arrival|overture|fanfare|call to|showtime|lights up|hot swing|sweeter vermouth)\b|커튼|개장|오프닝", 4),
    ...
    "A3_break": ("오프닝·신호", "브레이크 / 인터미션",
        r"\b(break|intermission|interlude|pause|breath(er)?|in[- ]between|sting|jingle|short cue|bass soli|vibe ace)\b|브레이크|인터미션|간주", 4),
    ...
    "B2_bebop": ("장르·결", "비밥",
        r"\b(bebop|be[- ]?bop|fast jazz|uptempo jazz|\bbop\b|dispersion relation|layabout|awesome call|faster does it)\b|비밥", 2),
    "B3_cool": ("장르·결", "쿨 재즈",
        r"\b(cool (jazz|vibes?)|west coast|mellow (jazz|vibe)|laid[- ]?back|understated|vibraphone|cool vibes)\b|쿨\s*재즈|쿨한", 5),
    "B4_hardbop": ("장르·결", "하드밥",
        r"\b(hard ?bop|gospel jazz|soulful jazz|cooking|blowing session|sidewalk shade|funky chunk|closer to jazz)\b|하드밥", 3),
    "B5_modal": ("장르·결", "모달",
        r"\b(modal|mode(s)? |kind of blue|impressionist|spacey jazz|open harmony|drone jazz|intractable|mellowtron|shades of spring)\b|모달", 5),
    "B6_latin": ("장르·결", "라틴 / 보사 / 삼바",
        r"\b(bossa|nova|samba|latin|mambo|rumba|cha[- ]?cha|afro[- ]?cuban|salsa|calypso|osaka|antigua|tropical|brazilian|ジャズボッサ)\b|보사|삼바|라틴|맘보", 5),
    ...
    "B9_smooth": ("장르·결", "스무스 / 컨템포러리",
        r"\b(smooth|easy (lemon|listening)|contemporary|lounge jazz|backed vibes|soft groove|chill jazz|elevator)\b|스무스|이지리스닝", 5),
    "B10_fusion": ("장르·결", "퓨전 / 일렉트릭",
        r"\b(fusion|electric jazz|jazz[- ]?rock|jazz[- ]?funk|synth jazz|rhodes|electric piano|nu[- ]?jazz|acid jazz|acidjazz|local forecast|funkorama|laser groove|thruster)\b|퓨전|일렉|애시드", 3),
    ...
    "B12_dixie": ("장르·결", "딕실랜드 / 트래디셔널",
        r"\b(dixie(land)?|trad(itional)? jazz|new orleans|ragtime|stride|hot jazz|trad jazz|clarinets?|fig leaf)\b|딕시|딕실랜드|래그타임|뉴올리언스", 3),
    "B13_soul": ("장르·결", "소울 재즈 / 펑키",
        r"\b(soul (jazz)?|funky|funk|groove|organ|hammond|boogie|get down)\b|소울|펑키|펑크|그루브|오르간", 3),
    "B14_noir": ("장르·결", "누아르 / 탐정",
        r"\b(noir|detective|hard[- ]?boiled|private eye|gumshoe|femme fatale|dances and dames|i knew a guy|spy|shadow(s|y)?|dark alley|crime|mystery jazz|film noir|investigations|darkest child)\b|누아르|탐정|느와르", 5),
    ...
    "C2_cafe": ("장소", "브런치 카페",
        r"\b(cafe|café|brunch|coffee|lobby|bistro|bakery|morning jazz|daytime|patio|terrace cafe|piano jazz)\b|카페|브런치|커피|로비", 5),
    ...
    "C6_studio": ("장소", "스튜디오 세션",
        r"\b(studio|session|rehearsal|trio|quartet|quintet|combo|practice|jam session|live take|bass vibes|hat the jazz|cool vibes)\b|스튜디오|세션|트리오|쿼텟", 5),
    "C7_dance": ("장소", "댄스플로어",
    ...
    "D3_uplift": ("감정·시간", "업리프트 / 선데이 모닝",
        r"\b(uplift(ing)?|sunday|spring|bright|cheerful|sunny|morning|optimistic|feel[- ]?good|happy|hopeful|気分上々)\b|업리프트|선데이|아침|봄|밝은", 4),
    ...
    "D6_elegant": ("감정·시간", "엘레강스 / 블랙타이",
        r"\b(elegant|black[- ]?tie|classy|sophisticated|glamour|glamorous|tuxedo|formal|deluxe|opulent|waltz|wallpaper|maple syrup jazz|古き良き|light up delight)\b|엘레강스|블랙타이|우아|세련", 4),
...
JAZZISH = re.compile(
    ...
    r"casa bossa|swinging pants|lobby time|sweeter vermouth|bass soli|bass vibes|"
    r"dispersion relation|layabout|awesome call|sidewalk shade|intractable|"
    r"funkorama|laser groove|atonal|formal waltz|fig leaf|hat the jazz|"
    r"재즈|ジャズ|ブルーズ|ラウンジ|ボサ"
    r")\b|"
    r"jazz|ジャズ|재즈|blues|블루스|swing|스윙|bossa|보사|lounge|라운지|funk|groove|rag|atonal",
    re.I,
)
...
TITLE_SCENE = {
    ...
    "sweeter vermouth": "A1_curtain",
    "hot swing": "A1_curtain",
    "bass soli": "A3_break",
    "vibe ace": "A3_break",
    "faster does it": "B2_bebop",
    "dispersion relation": "B2_bebop",
    "no good layabout": "B2_bebop",
    "nogoodlayabout": "B2_bebop",
    "be bop 25": "B2_bebop",
    "bebop 25": "B2_bebop",
    "awesome call": "B2_bebop",
    "sidewalk shade": "B4_hardbop",
    "funky chunk": "B4_hardbop",
    "closer to jazz": "B4_hardbop",
    "intractable": "B5_modal",
    "mellowtron": "B5_modal",
    "shades of spring": "B5_modal",
    "acidjazz": "B10_fusion",
    "acid jazz": "B10_fusion",
    "local forecast": "B10_fusion",
    "funkorama": "B10_fusion",
    "laser groove": "B10_fusion",
    "2017.12.08 atonal": "B11_free",
    "atonal in c": "B11_free",
    "atonal": "B11_free",
    "cool vibes": "C6_studio",
    "hat the jazz": "C6_studio",
    "bass vibes": "C6_studio",
    "wallpaper": "D6_elegant",
    "joey's formal waltz": "D6_elegant",
    "formal waltz": "D6_elegant",
    "古き良き日々": "D6_elegant",
    "maple syrup jazz": "D6_elegant",
    "light up delight": "D6_elegant",
    "fig leaf rag": "B12_dixie",
    "fig leaf times two": "B12_dixie",
}
