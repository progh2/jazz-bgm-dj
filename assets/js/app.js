...
function renderList() {
  ...
  if (!state.queue.length) {
    list.innerHTML = '<p class="empty">셋리스트가 비었습니다.</p>';
    return;
  }
...
function renderLedger(t) {
  const box = $('ledger');
  if (!t) { box.innerHTML = '<p class="empty">한 곡 틀기 시작하면 내력과 이용 허락이 여기 적힙니다.</p>'; return; }
...
function play(i) {
  ...
  const sr = $('sr-status');
  if (sr) sr.textContent = `재생 중: ${t.title}, ${t.artist}. 장면 ${SCENE_NAME[t.scene] || ''}. ${idx + 1}번째 곡, 모두 ${state.queue.length}곡.`;
...
function updateControls() {
  ...
  $('btn-play').setAttribute('aria-label', state.playing ? '멈추기' : '재생');
...
function dropCurrent(reason) {
  ...
  if (state.failStreak >= MAX_FAIL_STREAK) {
    stop();
    say(`${MAX_FAIL_STREAK}곡을 내리 틀지 못했습니다. 연결을 살펴봐 주십시오.`, 'talk');
    state.failStreak = 0;
    return;
  }
...
function renderWake() {
  ...
  if (lamp) {
    ...
    lamp.title = wakeSupported
      ? `등불 — ${label}. 켜 두면 재생하는 동안 화면이 꺼지지 않습니다`
  ...
  btn.addEventListener('click', async () => {
    ...
    if (state.wakeWanted) {
      ...
      say(wakeDenied
        ...
        : '재생하는 동안 등불을 켜 두겠습니다. 배터리는 좀 더 씁니다.');
...
function initSecrets() {
  secrets.onSecret((id, s, n) => {
    renderSecrets();
    say(`— ${s.label}. 클럽 야사에 적어 두었습니다. (${n}/${Object.keys(secrets.SECRETS).length})`, 'talk');
  });
  ...
  // 턴테이블을 건드리면 문답 없이 니키가 직접 셋을 짠다 — 지금 시각에 맞춰서
  ...
  secrets.onBecomePatron(() => {
    ...
    loadQueue(list);
    say('…끝까지 들어 주시는 분은 드뭅니다. 서른네 장면에서 가장 좋은 것만 뽑아 두었습니다. (아직 음원이 비어 있으면 기다려 주십시오)', 'talk');
    play(0);
  });
...
function initPhase() {
  ...
  $('phase-chip').addEventListener('click', () => {
    ...
    renderPhase(np);
    say(nextId ? `${np.label}의 클럽입니다.` : '실제 시각으로 되돌렸습니다.');
  });
}
...
const YT_ACCOUNT_KEY = 'jazzbgm.ytAccount';
...
function boot() {
  ...
  guardBardArt($('bard-stage'));   // 그림이 안 오면 SVG DJ 로 대신 세운다
  ...
  setScroll('DJ 니키에게 한 마디 건네면 골라 드립니다 —');
